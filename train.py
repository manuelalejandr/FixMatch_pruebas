...
if __name__ == '__main__':
    ...
    # create placeholders.
    x_image_var = tf.compat.v1.placeholder(tf.float32, [flags.batch_size] + [32, 32, 3])
    x_label_var = tf.compat.v1.placeholder(tf.float32, [flags.batch_size, 10])
    u_image_var = tf.compat.v1.placeholder(tf.float32, [flags.batch_size * flags.unlabeled_ratio, 2] + [32, 32, 3])
    ...
    # interleave
    skip_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
    ...
    # de-interleave
    post_ops = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS) if v not in skip_ops]
    ...
    pseudo_masks = tf.compat.v1.to_float(tf.greater_equal(tf.reduce_max(pseudo_labels, axis = -1), flags.confidence_threshold))
    ...
    learning_rate_ratio = tf.clip_by_value(tf.compat.v1.to_float(global_step) / flags.max_iteration, 0, 1)
    learning_rate = flags.init_learning_rate * tf.cos(learning_rate_ratio * (7 * np.pi) / (2 * 8)) 

    with tf.control_dependencies(post_ops):
        train_op = tf.compat.v1.train.MomentumOptimizer(learning_rate, momentum = 0.9, use_nesterov = True).minimize(loss_op, colocate_gradients_with_ops = True)
        train_op = tf.group(train_op, ema_op)
    
    #######################################################################################
    # 4. Test 
    #######################################################################################
    test_image_var = tf.compat.v1.placeholder(tf.float32, [None, 32, 32, 3])
    test_label_var = tf.compat.v1.placeholder(tf.float32, [None, 10])
    ...
    train_summary_op = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar(name, train_summary_dic[name]) for name in train_summary_dic.keys()])
